...
window.fetchUserProfile = async function () {
    ...
    try {
        const response = await fetch(`/users/${encodeURIComponent(currentUser)}`);
        if (response.ok) {
...
async function loadPostsList() {
    try {
        let url = '/media/?t=' + new Date().getTime();
        if (currentUser) {
...
window.deleteMedia = async function (mediaId, latLngKey) {
    ...
    try {
        const response = await fetch(`/media/${mediaId}?username=${currentUser}`, {
            method: 'DELETE'
        });
...
window.postComment = async function (mediaId, latLngKey) {
    ...
    try {
        const response = await fetch(`/comments/?media_id=${mediaId}&username=${currentUser}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.deleteComment = async function (commentId, latLngKey) {
    ...
    try {
        const url = `/comments/${commentId}?username=${currentUser}`;
...
window.toggleLike = async function (mediaId, latLngKey) {
    ...
    try {
        const response = await fetch(`/likes/${mediaId}?username=${encodeURIComponent(currentUser)}`, {
            method: 'POST'
        });
